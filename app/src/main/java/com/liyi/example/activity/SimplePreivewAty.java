...
public class SimplePreivewAty extends BaseActivity {
    ...
    private void initTagTransition() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            setExitSharedElementCallback(new SharedElementCallback() {
                ...
                public void onMapSharedElements(List<String> names, Map<String, View> sharedElements) {
                    // 页面切换监听
                    if (bundle != null) {// 重新进来的
                        int currentIndex = bundle.getInt("index", 0);
                        String url = bundle.getString("url", mImageList.get(0));
                        sharedElements.clear();
                        names.clear();
                        View currentView = autoGridView.getChildAt(currentIndex);
                        if (currentView != null) {
                            sharedElements.put(url, currentView);
                        }
                        bundle = null;
                    }
                }
...
//                }
                ...
                // 先做跳转
                Intent intent = new Intent(getBaseContext(), VPActivity.class);
                intent.putExtra("index", position);
                if (Build.VERSION.SDK_INT >= 22) {
                    ActivityOptionsCompat options = ActivityOptionsCompat
                            .makeSceneTransitionAnimation(SimplePreivewAty.this, view, mImageList.get(position));// mAdapter.get(position).getUrl()
